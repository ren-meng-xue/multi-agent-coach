...
from app.models.core import CandidateMemory, InterviewSession
from app.schemas.user import DashboardData, GrowthPoint, RadarData, WeaknessTag
...
async def get_user_dashboard_data(db: AsyncSession, user_id: str) -> DashboardData:
    ...
    # 练习时长只统计真正走到结束的 session：completed + 起止时间齐全。
    count_stmt = (
    ...
    # 2. 获取所有已完成会话用于计算时长；评分类统计单独跳过无评分场次。
    ...
    for s in all_sessions:
        if s.completed_at and s.started_at:
            ...
            total_duration_hours += max(0.0, delta.total_seconds() / 3600)
        if s.score is not None:
            scored_sessions.append(s)
            scores_sum += float(s.score)

    scored_count = len(scored_sessions)
    avg_score = round(scores_sum / scored_count, 1) if scored_count > 0 else 0.0
    
    # 3. 成长轨迹 (取最后 10 场)
    growth_trajectory = []
    # session_index 使用其在所有已完成 session 中的序号，曲线只展示已有评分的点。
    scored_session_ids = {s.id for s in scored_sessions}
    scored_session_indexes = {
        s.id: i + 1 for i, s in enumerate(all_sessions) if s.id in scored_session_ids
    }
    for s in scored_sessions[-10:]:
        growth_trajectory.append(GrowthPoint(
            session_index=scored_session_indexes[s.id],
            score=float(s.score or 0.0)
        ))
        
    # 4. 雷达图 (取最近 5 场平均值)
    radar = RadarData()
    recent_5 = scored_sessions[-5:]
    if recent_5:
